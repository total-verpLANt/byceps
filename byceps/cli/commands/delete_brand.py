...
from byceps.services.consent.dbmodels import DbConsentSubjectRequirement
...
def _count_consent_requirements(brand_id: BrandID) -> int:
    ...
    return (
        db.session.scalar(
            select(
                db.func.count(DbConsentSubjectRequirement.brand_id)
...
def _delete_brand_data(brand_id: BrandID, nullify_badges: bool) -> dict:
    ...
    try:
        ...
        consent_requirements_count = db.session.execute(
            delete(DbConsentSubjectRequirement).filter_by(brand_id=brand_id)
        ).rowcount
