...
def purge_entries_older_than(occurred_before: datetime) -> Result[int, str]:
    ...
    try:
        ...
        # delete_log_entries_older_than commits internally. If the
        # DELETE or that commit raises, the session is left in a
        # failed state where every later statement aborts. Roll it
        # back so a caller that keeps the session alive is not
        # handed a poisoned one along with the Err.
