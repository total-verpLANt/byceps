...
def test_validator_reports_keyless_contestant_row_instead_of_raising(repo):
    ...
    ``contestant_id()`` raises on such a row, and
    ``_distinct_contestant_count`` is only ever reached from
    ``validate_bracket_for_start``. Counting one unfiltered would turn
    "start tournament" -- a hard, non-overridable gate -- into an
    uncaught ValueError instead of a violation list, at the one moment
    an organiser cannot work around it.
    """
    from byceps.services.lan_tournament.tournament_match_service import (
        validate_bracket_for_start,
    )
    ...
    # One keyless row, as every DEFWIN-aware reader in the service
    # already expects to encounter.
...
def test_validator_reports_roundless_match_instead_of_raising(repo):
    ...
    ``TournamentMatch.round`` is ``int | None`` and the column is
    nullable. ``_validate_se_bracket`` derives the terminal round with
    ``max()`` over the main bracket; one ``None`` in that generator
    raises ``TypeError`` comparing ``NoneType`` to ``int``. Nothing
    catches it -- it escapes ``validate_bracket_for_start``, escapes
    ``change_status``, and 500s "start tournament" instead of
    producing the violation list this gate exists for. Same failure
    shape, and same fix, as the keyless contestant row above.
    """
    from byceps.services.lan_tournament.tournament_match_service import (
        validate_bracket_for_start,
    )
    ...
    # Strip the round from one first-round match, as a legacy or
    # hand-edited row would have it.
    ...
    # The rest of the bracket is still checked -- a roundless match
    # must not swallow the other findings, nor invent any.
...
def test_validator_still_finds_terminal_round_around_a_roundless_match(
    ...
    """The remaining matches are still validated against the real
    final round, not against a round the missing value shifted."""
    from byceps.services.lan_tournament.tournament_match_service import (
        validate_bracket_for_start,
    )
    ...
    # The roundless match is dropped from the round set; the final
    # must still be recognised as terminal, and a broken link on it
