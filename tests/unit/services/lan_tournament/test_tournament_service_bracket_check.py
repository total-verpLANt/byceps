...
def test_start_bracket_mode_without_matches_fails(
    ...
    # The ungenerated-bracket case reports the catalogued sentence,
    # not the composed 'Cannot start tournament: <diagnostics>' one:
    # the view flashes this string through gettext(), and only an
    # exact msgid gets translated. The composed form is reserved for
    # a bracket that exists but is structurally broken.
...
def test_resume_from_paused_skips_bracket_validation(
    ...
    ``validate_bracket_for_start`` asserts the topology a bracket has
    BEFORE play. Re-running it on PAUSED -> ONGOING would judge a
    bracket that normal play has already mutated.
    """
    from byceps.services.lan_tournament import tournament_service
...
def test_resume_from_paused_succeeds_with_wired_grand_final(
    ...
    That wiring is a start-validation violation ('grand final has
    next_match_id'), so before the gate was corrected a paused
    tournament that had produced a bracket reset could never resume.
    Drives the REAL validator to prove the gate, not a relaxed check,
    is what lets the resume through.
    """
    from byceps.services.lan_tournament import tournament_service
    ...
    mock_repository.get_tournament.return_value = tournament
    # An empty bracket would fail start validation outright; the point
    # is that validation never looks.
    mock_match_repo.get_matches_for_tournament_ordered.return_value = []
    mock_match_repo.get_contestants_for_tournament.return_value = {}
