...
def test_classify_unconfirmed_downstream(
    ...
    """An upstream match feeding an unconfirmed match is
...
def test_classify_confirmed_downstream(
    ...
    """An upstream match whose downstream match is confirmed is
...
def test_classify_confirmed_downstream_returns_full_transitive_chain(
    ...
    final). This is the exact scenario the false CONFIRMED_DOWNSTREAM
    warning text
...
def test_retraction_entry_records_cascaded_downstream_scores(
    ...
    """The retraction entry must carry the scores of every confirmed
    downstream match the cascade clears, not just the subject's.

    Those downstream results are gone from the database the moment
    the correction commits. If the audit entry does not record them,
    nothing does, and a mistakenly acknowledged CONFIRMED_DOWNSTREAM
    correction is unrecoverable -- which is the one situation the
...
def test_retraction_entry_cascade_scores_empty_without_downstream(
    ...
    """A correction with nothing confirmed downstream records an empty
    cascade snapshot -- present, so a reader never has to distinguish
    "no downstream" from "an older entry that predates the field"."""
    tournament = _create_tournament(
...
def test_correct_match_result_leaves_nothing_committed_when_correction_log_write_fails(
    ...
    staged 'match-result-corrected' log write must leave the DATABASE
    untouched -- exactly like the retraction-side failure above.

    correct_match_result is ONE transaction (workspace-ukkj). The
    retraction is staged by the flush-only ``_unconfirm_match_flush``
    and is NOT durable until the single ``commit_session()`` at the
    end of the function. The raise below happens before that commit,
    so the ``except Exception: rollback_session(); raise`` around the
    'match-result-corrected' write discards the retraction along with
    everything else: the match stays confirmed with its ORIGINAL
    scores, the corrected scores were never applied, and no log entry
    of either kind -- retraction or correction -- exists.

    This test previously asserted the opposite (a committed
    retraction). That was the older two-commit contract, in which
    unconfirm_match committed the retraction on its own before the
    corrected scores were applied by a second commit. workspace-ukkj
    removed that split precisely because a failure between the two
    commits left the bracket retracted with the correction unapplied,
    recoverable only by hand.

    The spy on rollback_session proves BOTH create_log_entry call
    sites in this module are wrapped identically. The fresh
    connection (see _read_via_fresh_connection) proves what is
    actually durable, rather than what the shared session happens to
    hold.
    """
    tournament = _create_tournament(
    ...
    # Production actually rolled back as a direct result of the raise
    # -- proves both create_log_entry call sites are wrapped
    # identically. This fails if the try/except around the
    # 'match-result-corrected' write is ever removed.
    assert len(rollback_calls) == 1

    # Nothing reached the single commit_session(), so a separate
    # connection sees the pre-correction state in full: still
    # confirmed, original scores intact, and no audit entry at all.
    fresh_match, fresh_scores, fresh_event_types = (
        _read_via_fresh_connection(tournament, match.id)
    )
    assert fresh_match.confirmed_by == admin_user.id
...
def test_correct_match_result_confirmed_downstream_requires_ack(
    ...
    """CONFIRMED_DOWNSTREAM refuses without ack; succeeds with ack
    and applies scores."""
    tournament = _create_tournament(
